  ...
  const handleSaveTweet = async () => {
    if (outputText && activeTab === "tweet") {
      try {
        ...
        await fetchSavedTweets(); // Refresh the tweets list
          ...
          {savedTweets.length > 0 ? (
            <ul className="space-y-3">
              {savedTweets.map((tweet) => (
                <li
                  key={tweet.id}
                  className="p-3 border border-gray-300 dark:border-gray-500 rounded-lg bg-white dark:bg-black relative group"
                >
                  <p className="text-gray-900 dark:text-white mb-2">{tweet.tweet_content}</p>
                  <div className="flex gap-2 mt-2">
                    <button
                      onClick={() => {
                        navigator.clipboard.writeText(tweet.tweet_content);
